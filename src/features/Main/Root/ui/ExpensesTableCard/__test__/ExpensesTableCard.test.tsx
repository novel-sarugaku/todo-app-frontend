...
describe('ExpensesTableCard', () => {
  describe('正常系', () => {
    it('合計ヘッダー内容と支出の合計額が正しく表示される（カンマ無しの小さい金額）', () => {
      customRender(<ExpensesTableCard {...defaultProps} />)

      expect(screen.getByText('2025年9月 合計支出額')).toBeInTheDocument()
    ...
    it('大きい金額はカンマ区切りで表示される', () => {
      ...
      customRender(<ExpensesTableCard {...mockBigAmountProps} />)

      expect(screen.getByText('50,200,000円')).toBeInTheDocument() // 5000000 + 200000
    })

    it('一覧テーブルのヘッダー内容と支出データが日付降順で正しく表示される', () => {
      customRender(<ExpensesTableCard {...defaultProps} />)
